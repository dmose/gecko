...
#ifndef mozilla_dom_Nimbus_Shared
#define mozilla_dom_Nimbus_Shared
...
struct nimbus_1725_ForeignBytes {
  int32_t mLen;
  const uint8_t* mData;
...
class MOZ_STACK_CLASS Reader final {
 ...
 private:
  ...
  const nimbus_1725_RustBuffer& mBuffer;
...
class MOZ_STACK_CLASS Writer final {
 public:
  Writer() {
    ...
    mBuffer = ffi_nimbus_1725_rustbuffer_alloc(0, &err);
    if (err.mCode) {
  ...
  /// Returns the buffer.
  nimbus_1725_RustBuffer Buffer() { return mBuffer; }
 ...
 private:
  ...
  nimbus_1725_RustBuffer mBuffer;
};
...
struct ViaFfi<nsACString, nimbus_1725_RustBuffer, false> {
  ...
  [[nodiscard]] static nimbus_1725_RustBuffer Lower(const nsACString& aLifted) {
    MOZ_RELEASE_ASSERT(
        aLifted.Length() <=
...
template <>
struct ViaFfi<nsAString, nimbus_1725_RustBuffer, false> {
  [[nodiscard]] static bool Lift(const nimbus_1725_RustBuffer& aLowered,
                                 nsAString& aLifted) {
    if (aLowered.mData) {
  ...
  [[nodiscard]] static nimbus_1725_RustBuffer Lower(const nsAString& aLifted) {
...
struct ViaFfi<T, nimbus_1725_RustBuffer> {
  ...
  [[nodiscard]] static nimbus_1725_RustBuffer Lower(const T& aLifted) {
    auto writer = Writer();
...
template <>
struct ViaFfi<nsACString, nimbus_1725_RustBuffer, true> {
  [[nodiscard]] static bool Lift(const nimbus_1725_RustBuffer& aLowered,
                                 nsACString& aLifted) {
...
template <>
struct ViaFfi<nsAString, nimbus_1725_RustBuffer, true> {
  [[nodiscard]] static bool Lift(const nimbus_1725_RustBuffer& aLowered,
                                 nsAString& aLifted) {
...
template <typename T>
struct ViaFfi<T, nimbus_1725_RustBuffer, true> {
  [[nodiscard]] static bool Lift(const nimbus_1725_RustBuffer& aLowered,
                                 T& aLifted) {
    auto reader = Reader(aLowered);
...
struct Serializable<AppContext> {
  ...
  static void WriteInto(Writer& aWriter, const AppContext& aValue) {
    Serializable<Optional<nsString>>::WriteInto(aWriter, aValue.mAppId);
...
struct Serializable<EnrolledExperiment> {
  ...
  static void WriteInto(Writer& aWriter, const EnrolledExperiment& aValue) {
    Serializable<nsString>::WriteInto(aWriter, aValue.mSlug);
    Serializable<nsString>::WriteInto(aWriter, aValue.mUserFacingName);
    Serializable<nsString>::WriteInto(aWriter, aValue.mUserFacingDescription);
    Serializable<nsString>::WriteInto(aWriter, aValue.mBranchSlug);
  }
};
template <>
struct Serializable<RemoteSettingsConfig> {
  [[nodiscard]] static bool ReadFrom(Reader& aReader,
    ...
    if (!Serializable<nsString>::ReadFrom(aReader, aValue.mBucketName)) {
      return false;
    }
    if (!Serializable<nsString>::ReadFrom(aReader, aValue.mCollectionName)) {
      return false;
    }
    return true;
  }

  static void WriteInto(Writer& aWriter, const RemoteSettingsConfig& aValue) {
    Serializable<nsString>::WriteInto(aWriter, aValue.mServerUrl);
    Serializable<nsString>::WriteInto(aWriter, aValue.mBucketName);
    Serializable<nsString>::WriteInto(aWriter, aValue.mCollectionName);
  }
};
template <>
...
#endif  // mozilla_dom_Nimbus_Shared