...
class AboutWelcomeParent extends JSWindowActorParent {
  ...
  didDestroy() {
    ...
    // To set ping values, we want a single layer for the ping.
    // Pop the reason and page out to top level. Note that we have
    // used the metrics.yaml file to define all of the fields in a way that
    // when send this object it will map with minimal additional effort.
    ping.reason = ping.event_context.reason;
    ping.page = ping.event_context.page;
  ...
  toCamelCase(s) {
    return s.toString().replace(/_([a-z])/gi, (str, group) => {
  ...
  setGleanMetricsAndSubmit(ping, type) {
    ...
    // todo: these come in snake_case, but need to be converted to camelCase
  ...
  async onContentMessage(type, data, browser) {
    ...
    switch (type) {
      ...
      case "AWPage:TELEMETRY_EVENT":
        ...
        if ("source" in Object.keys(objContext)) {
          ping.source = objContext.source;
        }
        if ("page" in Object.keys(objContext)) {
          ping.page = objContext.page;
        }
        delete ping.event_context;

        this.setGleanMetricsAndSubmit(data, type);
